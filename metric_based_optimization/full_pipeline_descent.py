...
def get_image(seed, iterations, prompt, metric,loss_scale = None):
    ...
    os.makedirs(
        os.path.join(output_dir,f"metric_optimization/{metric}/{prompt[0:45].strip()}/embeddings")
    )
    os.makedirs(os.path.join(output_dir,f"metric_optimization/{metric}/{prompt[0:45].strip()}/images"))
    score_list = list()
    ...
    for i in tqdm(range(int(iterations))):
        ...
        torch.save(
            gradient_descent.get_text_embedding(),
             os.path.join(output_dir,f"metric_optimization/{metric}/{prompt[0:45].strip()}/embeddings/{i}_{prompt[0:45].strip()}.pt"),
        )
        pil_image = ldm.latents_to_image(gradient_descent.latents)[0]
        pil_image.save(
             os.path.join(output_dir,f"metric_optimization/{metric}/{prompt[0:45].strip()}/images/{i}_{prompt[0:45].strip()}_{round(score.item(), 4)}.jpg")
        )
        ...
        if i == 0:
            pil_image = ldm.latents_to_image(gradient_descent.latents)[0]
            pil_image.save(
                 os.path.join(output_dir,f"metric_optimization/{metric}/{prompt[0:45].strip()}/initial_{prompt[0:45].strip()}_{round(optimized_score, 4)}.jpg")
            )
    ...
    with open(
         os.path.join(output_dir,f"metric_optimization/{metric}/{prompt[0:45].strip()}/{round(optimized_score, 4)}_output.txt"),
        "w",
    ) as file:
    ...
    pil_image = ldm.latents_to_image(gradient_descent.latents)[0]
    pil_image.save(
         os.path.join(output_dir,f"metric_optimization/{metric}/{prompt[0:45].strip()}/{prompt[0:45].strip()}_{round(optimized_score, 4)}.jpg")
    )
